...
|---|---|---|
| `languages: [hu, en]` | `microasis.locale` | one capture per language |
| `view: timetable\|now\|share` | `microasis.startView` | which iPhone tab opens |
| `view: settings` | `microasis.startView` | opens the Settings sheet |
| `view: widget` + `stage:` | `microasis.startWidgetPreview` | watch: render that stage's widget card full-screen |
| `empty: true` | `microasis.startNoProgram` | watch: force the "nothing on" card |
| `time: "yyyy-MM-dd HH:mm"` | `microasis.debugNow` | the debug clock (Budapest-local) |
| `location: <stage slug>` | `microasis.debugCoord` | nearest stage → left column (iPhone) / shown stage (watch); omitted → a far point so nothing is "nearby" |
| `columns: 1…5` | `microasis.columns` | iPhone timetable zoom (default 3) |
| `font: 1…5` | `microasis.fontSize` | text-size zoom (default 3) |
| *(always)* | `microasis.hideTestUI` | hides the in-app **Testing** settings section |
| *(always)* | `microasis.disclaimerSeen` | skips the first-launch modal |

`microasis.startView`, `microasis.hideTestUI` are invisible,
...
- **Widget shots** render the real `NowWidgetView` (shared via `OasisKit`) inside
  ...
  itself still ignores `microasis.debugNow`; only this in-app preview path uses it.
