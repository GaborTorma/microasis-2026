...
- **State:** `Settings`, `ScheduleStore`, `LocationStore` are `@MainActor`
  ...
  Group (`SharedDefaults`, `group.ai.torma.manas.2026`) each **app + its own widget
  extension** use (watch app + watch widget, iOS app + iOS widget — group containers
  are per-device, so this still shares nothing across devices) so the widget follows
  the app's language (and the QA `manas.debugNow` clock, DEBUG/TestFlight only), and
  — same group container — so the pair shares **one schedule disk cache** (the widget
  reuses the app's fetch).
...
- **widgets (both platforms):** `ManasNowWidget` (kind `"ManasNow"`,
  ...
  (`HomeWidgets.swift`) — both sizes render the watch card's "now" block, a divider
  under the act name, then as many up-next rows (act · start time) as fit
