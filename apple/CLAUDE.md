...
- **State:** `Settings`, `ScheduleStore`, `LocationStore` are `@MainActor`
  ...
  persisted as `manas.favoriteStates`; the widget process never activates
  `WCSession`). The App Group (`SharedDefaults`, `group.ai.torma.manas.2026`) the
  **watch app + its widget** share mirrors the app's language, the favorite slugs
  (`manas.favorites`), the QA `manas.debugNow` clock (DEBUG/TestFlight only), and —
  same group container — **one schedule disk cache** (the widget reuses the app's
  fetch) — same-device only.
