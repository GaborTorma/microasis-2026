...
struct MicrOasisWatchApp: App {
    ...
    /// Set `microasis.startWidgetPreview` to a stage slug. No-op in App Store builds.
    private var widgetPreviewSlug: String? {
        guard AppEnv.debugToolsEnabled,
              let slug = UserDefaults.standard.string(forKey: "microasis.startWidgetPreview"),
...
/// shot honours `microasis.debugNow` / `microasis.locale` without the widget extension
